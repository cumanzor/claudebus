...
cbus connect dev@nuc mbp --json    # inside the receiving Claude or Codex CLI
cbus list dev@nuc                  # one channel roster check
...
cbus connection disconnect dev@nuc/mbp  # retain local inbox and delivery history
```
...
- Endpoint autodetects loopback on the relay host; elsewhere configure
  `CBUS_SITE_NUC_URL` (for example `https://bus.example.com`). Credentials use
...
cbus connect bridge@nuc nuc --json
cbus list bridge@nuc              # check once after joining
cbus send bridge@nuc/mbp "hello from the NUC"

# --- on the Mac ---
cbus connect bridge@nuc mbp --json
cbus list bridge@nuc              # check once after joining
...
`cbus connection disconnect bridge@nuc/mbp`. That retains local mail and history.
