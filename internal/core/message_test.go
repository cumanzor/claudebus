...
func TestPeersResponseDecode(t *testing.T) {
	in := `{"dev/server":{"connected":true,"lastSeen":"2026-07-12T14:31:10.802809793-06:00","queued":3},` +
		`"x/y":{"connected":false,"lastSeen":"0001-01-01T00:00:00Z","queued":0}}`
	var pr PeersResponse
	if err := json.Unmarshal([]byte(in), &pr); err != nil {
		t.Fatal(err)
	}
	if e := pr["dev/server"]; !e.Connected || e.Queued != 3 || e.LastSeen.IsZero() {
		t.Errorf("dev/server = %+v, want connected/queued=3/non-zero lastSeen", e)
	}
