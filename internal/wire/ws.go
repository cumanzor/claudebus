...
const maxFrame = 2 << 20 // Allows the 1 MiB bus message plus a protocol envelope.
...
func Dial(addr, path, subprotocol string, timeout time.Duration) (*Conn, error) {
	...
	return handshakeClient(conn, addr, path, subprotocol, timeout)
}

// DialContext supports the relay's local ws and public wss front doors. Both
// transport setup and the upgrade are bounded and cancellation closes the fd.
func DialContext(ctx context.Context, rawURL, subprotocol string, timeout time.Duration) (*Conn, error) {
	u, err := url.Parse(rawURL)
	if err != nil || (u.Scheme != "ws" && u.Scheme != "wss") || u.Hostname() == "" || u.User != nil || u.Fragment != "" {
		return nil, errors.New("invalid relay WebSocket URL")
	}
	if timeout <= 0 {
	...
	return handshakeClient(conn, u.Host, u.RequestURI(), subprotocol, timeout)
}

func handshakeClient(conn net.Conn, addr, path, subprotocol string, timeout time.Duration) (*Conn, error) {
	...
	if subprotocol != "" && gotProto != subprotocol {
		conn.Close()
		return nil, errors.New("server did not echo the required subprotocol")
	}
...
// Abort closes immediately, including when a peer is not reading a close frame.
// Daemon cancellation uses this to unblock both readers and writers.
func (c *Conn) Abort() error { return c.c.Close() }
