...
func TestSiteURL(t *testing.T) {
	// no built-in hosts: an unset override is a hard error, even for server
	// (the promoted soft->hard delta)
	t.Setenv("CBUS_SITE_SERVER_URL", "") // isolate from shells that configure the fleet
	var uh *UnknownHostError
	if _, err := SiteURL("server"); !errors.As(err, &uh) {
		t.Errorf("SiteURL(server) with no override should be *UnknownHostError, got %v", err)
	}
	...
	t.Setenv("CBUS_SITE_SERVER_URL", "https://override.example")
	if u, err := SiteURL("server"); err != nil || u != "https://override.example" {
		t.Errorf("SiteURL(server) override = %q, %v", u, err)
	}
}
...
func TestResolveFrontDoor(t *testing.T) {
	// no built-in host: the public-mode branch resolves server via its env override
	t.Setenv("CBUS_SITE_SERVER_URL", "https://bus.example.com")
	...
	t.Run("exact ok -> local", func(t *testing.T) {
		...
		fd, err := ResolveFrontDoor("server")
