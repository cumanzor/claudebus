...
func TestSaveFormationRefreshPreservesHandEdits(t *testing.T) {
	...
	f.DriftAnchors = map[string]json.RawMessage{
		...
		"notes":    json.RawMessage(`"the tracker is re-queried, never trusted"`),
	}
	f.AnchorAlias = "coder"
	...
	if coder.Model != "opus" || coder.Rolefile != "roles/coder.md@b3a806e" || coder.Origin != OriginFresh ||
		coder.Mode != ModeResume || coder.OnStale != OnStaleFail || coder.Profile != "alpha" || coder.Target != "window" {
	...
	if string(f2.DriftAnchors["notes"]) != `"the tracker is re-queried, never trusted"` {
...
func TestSaveFormationHandAnchors(t *testing.T) {
	...
	f, _, err := SaveFormation("roles", "roles", map[string]string{"tracker": "trk-7m1", "note": "qa"})
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	if got := anchorStr(t, f, "tracker"); got != "trk-7m1" {
		t.Errorf("tracker anchor = %q", got)
	}
	...
	if got := anchorStr(t, f2, "tracker"); got != "trk-7m1" {
		t.Errorf("tracker anchor after flagless re-save = %q (hand keys must survive)", got)
	}
	...
	f3, _, err := SaveFormation("roles", "roles", map[string]string{"tracker": "trk-other"})
	if err != nil {
		t.Fatalf("re-save with flag: %v", err)
	}
	if got := anchorStr(t, f3, "tracker"); got != "trk-other" {
		t.Errorf("tracker anchor after explicit re-save = %q (flag must win)", got)
	}
...
func TestSaveFormationCapturesProfile(t *testing.T) {
	t.Setenv("CBUS_DIR", t.TempDir())
	t.Setenv("CLAUDE_CODE_SESSION_ID", "sid-coder") // the saver is a peer, so the mint resolves an anchor
	plantPeerProfile(t, "roles", "coder", "sid-coder", "beta")

	f, _, err := SaveFormation("roles", "roles", nil)
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	if f.Peers[0].Profile != "beta" {
		t.Errorf("captured profile = %q, want beta", f.Peers[0].Profile)
	}
	...
	plantPeerProfile(t, "roles", "coder", "sid-coder", "alpha")
	f3, _, err := SaveFormation("roles", "roles", nil)
	if err != nil {
		t.Fatalf("refresh save: %v", err)
	}
	if f3.Peers[0].Profile != "alpha" {
		t.Errorf("profile after refresh = %q, want alpha", f3.Peers[0].Profile)
	}
}
