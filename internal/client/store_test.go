...
func TestJoinStampsProfile(t *testing.T) {
	...
	t.Setenv("CLAUDE_CONFIG_DIR", filepath.Join(t.TempDir(), ".ccs", "instances", "beta"))
	if _, _, err := Join("dev", "worker"); err != nil {
		t.Fatal(err)
	}
	m, ok := ReadPeerMeta(filepath.Join(CBUSDir(), "dev", "worker", "meta.json"))
	if !ok || m.Profile != "beta" {
		t.Fatalf("profile = %q, want beta", m.Profile)
	}
}
...
func TestJoinProfileBlankOutsideCCS(t *testing.T) {
	...
	t.Setenv("CLAUDE_CONFIG_DIR", filepath.Join(t.TempDir(), "instances", "beta"))
...
func TestJoinStampsProfileTrailingSlash(t *testing.T) {
	...
	t.Setenv("CLAUDE_CONFIG_DIR", filepath.Join(t.TempDir(), ".ccs", "instances", "beta")+string(filepath.Separator))
	if _, _, err := Join("dev", "slashy"); err != nil {
		t.Fatal(err)
	}
	m, ok := ReadPeerMeta(filepath.Join(CBUSDir(), "dev", "slashy", "meta.json"))
	if !ok || m.Profile != "beta" {
		t.Fatalf("profile = %q, want beta despite trailing separator", m.Profile)
	}
}
