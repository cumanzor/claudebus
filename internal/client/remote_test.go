...
func TestRemoteListDecodes(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"dev/server":{"connected":true,"lastSeen":"2026-07-12T14:31:10.802809793-06:00","queued":2}}`))
	}))
	defer srv.Close()
	t.Setenv("CBUS_RELAY_LOCAL_URL", "http://127.0.0.1:1")
	t.Setenv("CBUS_SITE_TESTHOST_URL", srv.URL)
	...
	if e, ok := peers["dev/server"]; !ok || !e.Connected || e.Queued != 2 || e.LastSeen.IsZero() {
		t.Errorf("peers[dev/server] = %+v (ok=%v)", peers["dev/server"], ok)
	}
}
