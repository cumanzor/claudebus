...
func TestCodexSpawnIsIndependentOfTerminalAndParentHarness(t *testing.T) {
	...
	t.Setenv("CLAUDE_CONFIG_DIR", "/parent/.ccs/instances/beta")
	...
	for _, target := range []string{"window", "tab", "tmux", "pane"} {
		f := &fakeForker{}
		_, alias, err := SpawnWithOptions(target, "codex-test", "gpt-5", "worker-"+target, "", SpawnOptions{Harness: "codex", Profile: "beta"}, f)
		if err != nil {
			t.Fatal(err)
		}
		...
		for _, v := range []string{"--profile", "beta", "--model", "gpt-5", "CODEX_THREAD_ID", "CLAUDE_CODE_SESSION_ID", "CODEX_EXEC_SERVER_URL"} {
