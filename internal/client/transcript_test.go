...
func TestTranscriptPathProfileSibling(t *testing.T) {
	...
	t.Setenv("CLAUDE_CONFIG_DIR", alpha)
	setHome(t, home)
	sid := "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee"
	want := writeTranscript(t, beta, "-Users-dev-work-repo", sid)

	if _, ok := TranscriptPath("", sid); ok {
		t.Error("without the profile hint the beta-instance transcript must not resolve")
	}
