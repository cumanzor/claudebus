...
func TestKeychainBackendArgv(t *testing.T) {
	...
	m := &mockRunner{}
	if err := (keychainBackend{run: m}).put("server", "token", "SECRET"); err != nil {
		t.Fatal(err)
	}
	if m.name != "security" || !slices.Equal(m.args, []string{"-i"}) {
		t.Fatalf("put argv = %q %q, want security [-i]", m.name, m.args)
	}
	wantStdin := `add-generic-password -U -s "cbus-relay-server" -a "token" -w "SECRET"` + "\n"
	...
	v, _ := (keychainBackend{run: m2}).get("server", "cf-id")
	wantArgs := []string{"find-generic-password", "-s", "cbus-relay-server", "-a", "cf-id", "-w"}
	...
	if v, _ := (keychainBackend{run: m3}).get("server", "token"); v != "" {
...
func TestKeychainBackendExplicitKeychain(t *testing.T) {
	m := &mockRunner{}
	_ = (keychainBackend{run: m, keychain: "/tmp/it.keychain"}).put("server", "token", "S")
	wantStdin := `add-generic-password -U -s "cbus-relay-server" -a "token" -w "S" "/tmp/it.keychain"` + "\n"
	if m.stdin != wantStdin {
		t.Fatalf("put stdin = %q, want %q", m.stdin, wantStdin)
	}
	m2 := &mockRunner{out: "S\n"}
	_, _ = (keychainBackend{run: m2, keychain: "/tmp/it.keychain"}).get("server", "token")
...
func TestFileBackend(t *testing.T) {
	...
	fb := fileBackend{}
	if err := fb.put("server", "token", "abc123"); err != nil {
		t.Fatal(err)
	}
	if v, _ := fb.get("server", "token"); v != "abc123" {
		t.Fatalf("round-trip get = %q", v)
	}
	if v, _ := fb.get("server", "cf-id"); v != "" {
...
func TestFileBackendPermissions(t *testing.T) {
	...
	dir := t.TempDir()
	t.Setenv("XDG_CONFIG_HOME", dir)
	if err := (fileBackend{}).put("server", "token", "abc123"); err != nil {
		t.Fatal(err)
	}
	if fi, _ := os.Stat(filepath.Join(dir, "cbus", "server")); fi.Mode().Perm() != 0o700 {
		t.Errorf("dir perm = %o, want 700", fi.Mode().Perm())
	}
	ffi, _ := os.Stat(filepath.Join(dir, "cbus", "server", "token"))
