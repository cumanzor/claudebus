...
func TestClaudeLaunchCommandsPreserveStoreAndClearTerminalIdentity(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.launch, func(t *testing.T) {
			...
			config := ""
			if tc.profile {
				config = filepath.Join(home, ".ccs", "instances", "personal")
			}
...
func TestClaudeLaunchPinsRelativeRootsBeforeChangingDirectory(t *testing.T) {
	...
	t.Setenv("CLAUDE_CONFIG_DIR", filepath.Join(".ccs", "instances", "personal"))
	env, err = peerEnv("work")
	if err != nil {
		t.Fatal(err)
	}
	if env["CLAUDE_CONFIG_DIR"] != filepath.Join(cwd(), ".ccs", "instances", "work") {
