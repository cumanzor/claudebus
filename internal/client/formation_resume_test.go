...
func resumeFixture() *Formation {
	return &Formation{
		...
		Peers: []FormationPeer{{
			...
			Mode: ModeTemplate, Machine: "host-a", Profile: "beta",
...
func TestResumeAnchorLaunchShape(t *testing.T) {
	...
	if len(argv) < 5 || argv[0] != "ccs" || argv[1] != "beta" {
		t.Fatalf("argv prefix = %v, want ccs beta", argv[:2])
	}
...
func fleetFixture() *Formation {
	return &Formation{
		...
		Peers: []FormationPeer{
			{Alias: "orchestrator", SessionID: "sid-anchor", Origin: OriginJoined,
				Mode: ModeTemplate, Machine: "host-a", Profile: "beta", Cwd: "/nonexistent/recorded/cwd"},
			{Alias: "documenter", SessionID: "sid-warm", Origin: OriginFresh,
				Mode: ModeResume, Machine: "host-a", Profile: "beta"},
			{Alias: "reviewer", SessionID: "sid-gone", Origin: OriginFresh,
				Mode: ModeResume, Machine: "host-a", Profile: "beta"},
			{Alias: "tester", SessionID: "sid-remote", Origin: OriginFresh,
				Mode: ModeResume, Machine: "host-b", Profile: "beta"},
...
func TestResumeAnchorArgvShapeUnchanged(t *testing.T) {
	...
	argv := fk.specs[0].Argv
	want := []string{"ccs", "beta", "--resume", "sid-anchor", "--name", "orchestrator"}
...
func TestAnchorKickoffExampleContract(t *testing.T) {
	...
	three := &Formation{
		...
		Peers: []FormationPeer{
			...
			{Alias: "documenter", SessionID: "sid-w1", Origin: OriginFresh, Mode: ModeResume, Machine: "", Profile: "beta"},
			{Alias: "reviewer", SessionID: "sid-w2", Origin: OriginFresh, Mode: ModeResume, Machine: "host-a", Profile: "beta"},
			{Alias: "tester", SessionID: "sid-w3", Origin: OriginFresh, Mode: ModeResume, Machine: "host-a", Profile: "beta"},
		},
	}
...
func TestAnchorRosterMirrorsSidState(t *testing.T) {
	home := t.TempDir()
	cfg := filepath.Join(home, ".ccs", "instances", "alpha")
...
func TestResumeAnchorInfersProfileFromSweep(t *testing.T) {
	...
	home := t.TempDir()
	setHome(t, home)
	t.Setenv("CLAUDE_CONFIG_DIR", filepath.Join(home, ".ccs", "instances", "alpha"))
	f := resumeFixture()
	f.Peers[0].Profile = ""
	w := resumeWorld()
	w.HasTranscript = func(profile, sid string) bool { return profile == "beta" }
	...
	if created != "surface-1" || inferred != "beta" {
		t.Errorf("created=%q inferred=%q, want surface-1 / beta", created, inferred)
	}
	argv := fk.specs[0].Argv
	if argv[0] != "ccs" || argv[1] != "beta" {
		t.Errorf("argv prefix = %v, want the inferred profile, not the env's", argv[:2])
	}
	if env := fk.specs[0].Env["CLAUDE_CONFIG_DIR"]; !strings.HasSuffix(env, filepath.Join("instances", "beta")) {
		t.Errorf("child CLAUDE_CONFIG_DIR = %q, want the inferred beta instance", env)
	}
...
func TestResumeAnchorSweepRefusals(t *testing.T) {
	for _, tc := range []struct {
		...
		{"ambiguous", []string{"alpha", "beta"}, "cannot pick"},
...
func TestResumeAnchorRecordedProfileNeverSweeps(t *testing.T) {
	t.Setenv("CBUS_DIR", t.TempDir())
	f := resumeFixture() // Profile "beta" is recorded
	w := resumeWorld()
	w.HasTranscript = func(string, string) bool { return false }
...
// mobile-scan shape that filed cbus-kl4. Real InstanceProfiles plus the
...
func TestResumeAnchorSweepEndToEnd(t *testing.T) {
	...
	writeTranscript(t, filepath.Join(home, ".ccs", "instances", "beta"), "-Users-dev-work-repo", sid)
	...
	if inferred != "beta" {
		t.Errorf("inferred = %q, want beta", inferred)
	}
