...
func TestSpawnFreshArgvCCSProfile(t *testing.T) {
	...
	if f.spec.Argv[0] != "ccs" || f.spec.Argv[1] != "alpha" {
...
func TestSpawnRemoteAddress(t *testing.T) {
	f := &fakeForker{}
	addr, child, err := Spawn("tab", "dev@server", "", "", "", f)
	if err != nil {
		t.Fatal(err)
	}
	if addr != "dev@server" {
	...
	if i := slices.Index(f.spec.Argv, "--name"); i < 0 || f.spec.Argv[i+1] != "dev@server" {
		t.Fatalf("remote default title should be the address: %v", f.spec.Argv)
	}
	if f.spec.Title != "dev@server" {
...
func TestSpawnRejectsAliasAndBadNames(t *testing.T) {
	...
	for _, tc := range []struct{ target, addr, wantErr string }{
		...
		{"window", "dev@server/laptop", "no alias"},
		{"split", "dev", "target must be"},
		{"window", "a b", `bad channel "a b"`},
		{"window", "dev@", `bad host ""`},
		{"window", "@server", `bad channel ""`},
	} {
...
func TestSpawnNameFixesAlias(t *testing.T) {
	...
	f = &fakeForker{}
	_, child, err = Spawn("tab", "dev@server", "", "laptop2", "", f)
	if err != nil {
		t.Fatal(err)
	}
	if child != "laptop2" {
	...
	if got := f.spec.Argv[len(f.spec.Argv)-1]; got != SpawnPromptAliased("dev@server", "laptop2") {
		t.Fatalf("remote prompt = %q", got)
	}
	if f.spec.Title != "laptop2" {
		t.Fatalf("remote --name Title (tmux window name) = %q, want laptop2", f.spec.Title)
	}
