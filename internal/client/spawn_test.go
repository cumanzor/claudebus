...
func TestSpawnRemoteAddress(t *testing.T) {
	...
	for _, want := range []string{"cbus connect dev@nuc", "your-explicit-alias", "do not start a Monitor", "cbus list dev@nuc"} {
...
func TestSpawnPromptAliasedContent(t *testing.T) {
	...
	for _, want := range []string{"cbus connect dev@nuc mbp2", "cbus list dev@nuc", "do not start a Monitor"} {
