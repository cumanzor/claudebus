...
// ---- large pids (F4) --------------------------------------------------------------
//
// The server runs pid_max 4194304, so 7-digit pids are ordinary there. darwin caps at
...
// largePid is a realistic server pid: past the 1e6 point where fmt's %v for float64
...
// server, and pins the boundary so the next person does not have to rediscover it: the
// float64 path is exact for every pid darwin can allocate and lossy from 1e6 up, which
// is squarely inside the server's range.
...
// test-only, but that had been demonstrated once by hand on the server and asserted
