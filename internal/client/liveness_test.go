...
func TestReadPeerMeta(t *testing.T) {
	...
	_ = os.WriteFile(mp, []byte(`{"listenerPid":1234,"ownerPid":null,"host":"laptop","cwd":"/x","alias":"a"}`), 0o644)
	if m, ok := ReadPeerMeta(mp); !ok || m.ListenerPid != 1234 || m.OwnerPid != 0 || m.Host != "laptop" || m.Cwd != "/x" {
