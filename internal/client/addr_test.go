...
func TestIsRemote(t *testing.T) {
	remote := []string{"dev@server/laptop", "@server", "a@", "dev@server"}
...
func TestParseRemote(t *testing.T) {
	cases := []struct {
		...
		{"dev@server/laptop", "dev", "server", "laptop", false, ""},
		{"dev@server", "dev", "server", "", false, ""},
		{"@server/al", "", "server", "al", false, ""}, // empty channel accepted client-side
		{"@server", "", "server", "", false, ""},
		{"dev@server/a/b", "", "", "", true, `bad alias "a/b"`},
...
func TestParse(t *testing.T) {
	...
	if tg, err := Parse("dev@server/laptop", nil); err != nil || !tg.Remote || tg.Channel != "dev" || tg.Host != "server" || tg.Alias != "laptop" {
