...
CLAUDE_CODE_SESSION_ID=OTHER "$GO" join dev laptop  >/dev/null
CLAUDE_CODE_SESSION_ID=SGO   "$GO" send dev/laptop "hello from go" >/dev/null
echo "  go wrote dev/me (SGO) + dev/laptop (OTHER) + a send + join-presence"
...
has "$L" "dev/laptop" && ok "bash list sees dev/laptop" || no "bash list dev/laptop"
...
I=$(CLAUDE_CODE_SESSION_ID=SGO "$BASH_CBUS" inbox dev/laptop)
has "$I" "dev/laptop/inbox.jsonl" && ok "bash inbox path on go peer" || no "bash inbox"

echo "=== bash cbus OPERATES on the go-written state ==="
CLAUDE_CODE_SESSION_ID=SGO "$BASH_CBUS" send dev/laptop "hello from bash rollback" >/dev/null
INB=$(cat "$CBUS_DIR/dev/laptop/inbox.jsonl")
...
CLAUDE_CODE_SESSION_ID=BR "$BASH_CBUS" unregister dev/laptop >/dev/null
[ ! -d "$CBUS_DIR/dev/laptop" ] && ok "bash unregister removes go peer" || no "bash unregister"
