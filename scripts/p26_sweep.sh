...
joined_peer() { # a live-ish peer 'dev/laptop' owned by another session + a witness
  CLAUDE_CODE_SESSION_ID=OTHER "$1" join dev laptop >/dev/null 2>&1
  CLAUDE_CODE_SESSION_ID=SID   "$1" join dev me  >/dev/null 2>&1
}
two_peers() {
  CLAUDE_CODE_SESSION_ID=OTHER "$1" join dev laptop >/dev/null 2>&1
  CLAUDE_CODE_SESSION_ID=OTHER "$1" join dev server >/dev/null 2>&1
}
...
cmp_verb "inbox"             noop        inbox dev/laptop
cmp_verb "inbox-bare-refuse" noop        inbox bare
cmp_verb "auth-status"       noop        auth status server
...
cmp_verb "send-to-peer"      joined_peer send dev/laptop hello world
cmp_verb "send-bare-alias"   joined_peer send laptop hi
cmp_verb "send-empty"        joined_peer send dev/laptop
cmp_verb "leave"             joined      leave
cmp_verb "leave-not-joined"  noop        leave
cmp_verb "rename"            joined      rename newname
cmp_verb "unregister"        joined_peer unregister dev/laptop
