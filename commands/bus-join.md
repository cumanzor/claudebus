...
1. **Pick the channel**: use the one the user passed, else the git repo's
   ...
   **Remote channel?** If the channel contains `@` (e.g. `dev@server`), it's
   relay-backed and cross-machine: pick an explicit alias (short hostname/role,
   e.g. `laptop`), run `cbus tail <channel>/<alias>` (full form `dev@server/laptop`) to
   ...
   `cbus auth set <host>` with values from a password manager.
