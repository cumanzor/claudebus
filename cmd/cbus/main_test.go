...
func TestAuthSetAndStatus(t *testing.T) {
	...
	out := captureStdout(t, func() {
		if rc := runAuthSet(store, []string{"server", "--token", "  abc123  ", "--cf-id", "cf-xyz"}, strings.NewReader("")); rc != 0 {
			t.Fatalf("runAuthSet rc=%d", rc)
		}
	})
	if !strings.Contains(out, "stored 2 credential(s) for server") {
		t.Errorf("set output = %q", out)
	}
	// whitespace stripped on write
	if b, _ := os.ReadFile(filepath.Join(dir, "cbus", "server", "token")); string(b) != "abc123" {
		t.Errorf("stored token = %q, want abc123", b)
	}

	st := captureStdout(t, func() { runAuthStatus(store, []string{"server"}) })
	for _, want := range []string{"site server:", "set (…c123)", "set (…-xyz)", "absent"} {
...
func TestAuthSetStdin(t *testing.T) {
	...
	if rc := runAuthSet(store, []string{"server", "--token", "-"}, strings.NewReader("tok-from-stdin\n")); rc != 0 {
		t.Fatalf("rc=%d", rc)
	}
	if b, _ := os.ReadFile(filepath.Join(dir, "cbus", "server", "token")); string(b) != "tok-from-stdin" {
...
func TestAuthSetErrors(t *testing.T) {
	...
	cases := map[string][]string{
		"unknown flag":   {"server", "--bogus", "x"},
		"empty value":    {"server", "--token", "   "},
		"nothing to set": {"server"},
		"bad host":       {"bad/host", "--token", "x"},
		"missing value":  {"server", "--token"},
	}
...
func TestAuthSetDoubleStdinDrainsOnce(t *testing.T) {
	...
	t.Setenv("XDG_CONFIG_HOME", dir)
	store := client.NewFileCredStore()
	rc := runAuthSet(store, []string{"server", "--token", "-", "--cf-id", "-"}, strings.NewReader("only-one-token\n"))
	if rc == 0 {
		t.Error("second '-' should drain empty and fail")
	}
	if b, _ := os.ReadFile(filepath.Join(dir, "cbus", "server", "token")); string(b) != "only-one-token" {
		t.Errorf("first token = %q, want only-one-token", b)
	}
	if _, err := os.Stat(filepath.Join(dir, "cbus", "server", "cf-id")); err == nil {
...
func TestWhoami(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(root, ".remote", "server", "dev"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, ".remote", "server", "dev", "SID"), []byte(`{"alias":"laptop"}`), 0o644); err != nil {
		t.Fatal(err)
	}
	out := captureStdout(t, func() {
	...
	if !strings.Contains(out, "dev@server/laptop (remote from-default — reachability: cbus list @server)") {
...
func TestInboxPathNoNewline(t *testing.T) {
	...
	t.Setenv("CBUS_DIR", root)
	out := captureStdout(t, func() {
		if rc := runInbox([]string{"dev/laptop"}); rc != 0 {
			t.Fatalf("rc=%d", rc)
		}
	})
	if want := filepath.Join(root, "dev", "laptop", "inbox.jsonl"); out != want {
...
func TestChannelsAndListLocal(t *testing.T) {
	root := t.TempDir()
	t.Setenv("CBUS_DIR", root)
	seedPeer(t, root, "dev", "laptop", `{"listenerPid":null,"host":"laptop","cwd":"/w"}`)
	seedPeer(t, root, "dev", "server", `{"listenerPid":null,"host":"server","cwd":"/x"}`)
	...
	if !strings.Contains(li, "dev/laptop") || !strings.Contains(li, "pid=?") || !strings.HasPrefix(li, "off   ") {
		t.Errorf("list = %q", li)
	}
...
func TestRenderRemoteList(t *testing.T) {
	...
	peers := core.PeersResponse{
		"dev/server": {Connected: true, Queued: 0, LastSeen: mustTime("2026-07-12T14:31:10.802809793-06:00")},
		"dev/laptop": {Connected: false, Queued: 3, LastSeen: mustTime("2026-07-12T15:00:00Z")},
		"other/x":    {Connected: true, Queued: 1, LastSeen: mustTime("2026-07-12T15:00:00Z")},
	}
	// channel filter "dev" excludes other/x; sorted -> dev/laptop before dev/server
	out := captureStdout(t, func() { renderRemoteList(peers, "dev", "server") })
	lines := strings.Split(strings.TrimRight(out, "\n"), "\n")
	if len(lines) != 2 {
		t.Fatalf("expected 2 filtered lines, got %d:\n%s", len(lines), out)
	}
	if !strings.HasPrefix(lines[0], "off   ") || !strings.Contains(lines[0], "dev@server/laptop") || !strings.Contains(lines[0], "queued=3") {
		t.Errorf("line0 = %q", lines[0])
	}
	if !strings.HasPrefix(lines[1], "listen") || !strings.Contains(lines[1], "dev@server/server") || !strings.Contains(lines[1], "lastSeen=2026-07-12T14:31:10.802809793-06:00") {
		t.Errorf("line1 = %q", lines[1])
	}

	// empty result prints the "no remote peers in ..." line
	empty := captureStdout(t, func() { renderRemoteList(core.PeersResponse{}, "dev", "server") })
	if strings.TrimSpace(empty) != "no remote peers in dev@server" {
