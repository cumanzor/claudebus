...
// close is a phase-1 windows-excluded verb (unsupported_windows.go): runClose refuses
// ahead of resolution on that build, so none of these resolution/refusal/reporting
// assertions can hold there and the whole matrix is unix-only. Windows close coverage is
// the honest refusal in unsupported_windows_test.go. Every test here drives the real CLI
// entry (run/runClose) and every peer it seeds has ownerPid null or a session-id that
// trips the self-refusal, so the matrix exercises resolution, refusal and reporting
...
// command", which is the failure this pins. On windows close is dispatched to its
// phase-1 refusal (unsupported_windows_test.go), not to this usage line, so the
