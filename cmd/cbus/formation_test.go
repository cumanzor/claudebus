...
// fail everywhere else — including the server, where a release rebuild runs it.
...
func TestFormationSaveAnchorFlag(t *testing.T) {
	...
	if rc := runFormation([]string{"save", "far", "roles", "--anchor", "tracker=trk-7m1", "--anchor", "note=qa"}); rc != 0 {
	...
	for _, want := range []string{`"tracker": "trk-7m1"`, `"note": "qa"`} {
	...
	if rc := runFormation([]string{"save", "near", "--anchor", "tracker=trk-7m1"}); rc != 0 {
