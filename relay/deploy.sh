#!/usr/bin/env bash
# deploy the relay to the server: rsync source, build with the server's Go, seed a
# token if missing, install + start the systemd unit. Idempotent.
set -euo pipefail

HOST="${CBUS_RELAY_HOST:-server}"
...
# ship the whole module so it stays self-contained on the server: root go.mod,
