...
> one specific setup (a small always-on home server reachable through an authenticated tunnel). It's here to
...
`--anchor tracker=<id>`, and a personal dashboard renders formations by reading
...
that same private tracker by opaque ids (`cbus-xxx`, `trk-xxx`). They are
...
- **Cross-machine relay — keep it off the open internet.** The relay is a single-operator
  ...
  the daemon to `127.0.0.1` and fronting it with an edge access-control layer.
