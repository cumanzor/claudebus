...
[Motivating problem]
Carlos tried to resume the `mobile-app-fleet` formation after its tmux session was
gone. From a bare shell:

    cbus formation resume 'mobile-app-fleet'
...
`.ledger/mobile-app-channel.jsonl` for that day, and one bare-zsh tmux pane
...
- NOT field-smoked. The repo rule is to drive a real formation before commit, and the
  live half here is Carlos re-running `cbus formation resume 'mobile-app-fleet'` from
...
- Mac: 0.11.1 -> 0.11.2. Server: same over ssh, `~/.local/bin/cbus` hashing to the
...
- Field smoke on the REAL store (`~/.claude-bus`), not a fixture: `cbus list`
  ...
  listeners (shared-mobile-client, cbus-winport/orchestrator) stayed live
...
- `cbus selfupdate` rewrites the installed roles at `~/.claude-bus/roles` from
  ...
  them live.
- Fleet after propagation: mac and server on v0.11.0, winbox still on v0.10.2 until
  it runs `cbus selfupdate`. The server came from v0.10.1, not v0.10.2 (it never
...
- CORRECTION (same day): the two reviewer-cleared v0.10.2 follow-up patches are
  ...
  still deferred and gated on a winbox narrow-scope 8-run.
...
## [2026-09-05 00:45:00 UTC] [Release/Windows] v0.10.2 SHIPPED: native Windows cbus, phase 1, released and installed on winbox end to end
...
[Motivating problem]
Windows-native Claude Code sessions (winbox) had no way to join the fleet: cbus
...
Rows 0-6 all green on winbox itself: windows test binaries pass by name (cmd
...
installed on winbox at `%USERPROFILE%\.local\bin\cbus.exe`; `--version` reports
...
Found during the credential ACL inventory on winbox with a throwaway token;
...
independent verification, or a specific tracker item (`cbus-que.4`/`.6`/`.18`/`.21`/`.23`),
...
missed their fixture entirely and read as red on winbox -- the earlier staged reading
...
- The residue this leak actually wrote on winbox was located and removed: one 69-byte
  cache file plus the empty directory T4 had created around it, with zero credential
  files found beside it. Recorded on `cbus-que.18`, not this tracker item.
...
and run with P5 -- rather than a bare tracker item reference or an inventory the skip text
...
the accident of hitting the refusal first. 8 such tests were red on winbox; 4 were
...
- With this commit and C1(3) commit A together, `cbus-que.11` is code-complete: the
  ...
  in the T4 baseline is now accounted for by class. The tracker item itself closes on the
  candidate winbox run, not on this commit.
- Not independently re-run by documenter; verified by reading the diff and commit
  message against the tree.
...
[Motivating problem]
Six tests build the real cbus binary at runtime and go red on the winbox gate host,
...
[Possible Ripple Effects]
- By mechanism, not coincidence: the six skips on the toolchain-less winbox host
...
- `cmd/cbus/main_test.go` (`captureStdout`) and `cmd/cbus/update_check_test.go`
  ...
  helpers only; all call sites (103 across 17 files, per the tracker item's corrected count --
...
- Not run on winbox as part of this entry -- C1(1) fixes the mechanism; the close
  matrix's own winbox pass belongs to C1(2)'s report.
...
- $CBUS_DIR/roles/{coder,orchestrator,tester}.md on the laptop and the server
  (tester is a runtime-only role with no repo copy).
- Saved formation envelopes: `"model": "opus"` -> `"model": "claude-opus-4-8"`,
  33 fields across 17 laptop .formations JSONs + 3 fields in the server's; every
...
- The port now sits on the v0.9.3 feature surface (resume/--mode/anchors); windows
  ...
  the suite, but winbox has not seen this tree yet.
...
- The reclaim lock on windows now takes LockFileEx byte-range semantics rather than
  ...
  compile-verified only, first-executes on winbox.
...
formation: `cbus formation resume 'mobile-scan'` from a bare shell
refused with "aged out or lives under another profile" while the transcript sat
1.6MB under ~/.ccs/instances/<profile>. Filed as cbus-kl4 with a store census: 16 of
17 envelopes record an empty profile on every peer (only feature-rollout, re-saved
post-capture, records one) -- including feature-cleanup saved the day before, because
...
mobile-scan shape. Two mutants killed on their aimed assertions:
...
- Refuse-over-autopick on mints; warn-and-save on refreshes; heal on joined
  re-saves. Three paths, three different answers, each reasoned on the tracker item.
...
[Motivating problem]
The feature-rollout reboot proved the resume story end to end but left two gaps: choosing
...
[Motivating problem]
Field-proven on the feature-rollout reboot: the envelope records everything needed to
resume EXCEPT the CCS profile (so the correct `ccs beta --resume` had to be derived
...
- Envelopes saved by pre-yv3 binaries have blank profiles; the resume verb then
  depends on the invoking shell's own profile to find transcripts. feature-rollout was
  hand-filled to profile=beta (legal hand fill, preserved by blank-never-clobbers).
...
- Real-store smoke: `resume ghost` errors; `resume feature-rollout` with its anchor
  LIVE drew the live-armed refusal naming feature-rollout/orchestrator — after the
...
(anchor precedence shipped dashboard-side in trk-7m1.3). The key already
...
- `cmd/cbus/usage.go` — save block documents the flag and the tracker convention.
...
- Repeated valued flags on OTHER verbs still last-wins silently, as before; only
  save reads `all()`.
- The dashboard sweep reads `drift_anchors.tracker` as the direct epic link with
...
- Read-only against the real store: 14 envelopes list, `mobile-foundry` renders
...
[Ripple]
None filed. This tracker item's scope (asset naming, release guard, self-replacement) is now
...
be confirmed against the actual gate, per the tracker item's own acceptance criterion, separately
...
its own tracker item deliberately rather than folded under the windows-scoped
`cbus-que.10`: this is a PRODUCTION defect on every platform, and burying a
fleet-wide finding under a windows-numbered tracker item is exactly the
...
- `internal/client/cursor.go` -- `writeCursor` changes signature from no return
  ...
  it would not prejudge this tracker item's design. New `warnCursorWriteOnce(peerDir,
...
- ACCEPTANCE, registered before the run specifically so it could falsify
  ...
  scope, same 8 runs, on winbox. Editing those tests as part of this fix would
...
- `cbus-que.14` (the `.stop-cursor` sidecar, a separate cursor file for the
  ...
  enumeration) inherits this exact fix shape for whenever that tracker item is
...
`cbus-que.13`; new tracker item `cbus-que.14`.
...
write, "mechanism entirely uncharacterised" per the tracker item's own opening
...
- STARTING POINT: cluster B (filed to this tracker item in M5) was three, later four,
...
- The `.stop-cursor` sidecar (a separate cursor file for the codex Stop-hook
  ...
  pass -- filed as new tracker item `cbus-que.14` rather than assumed clear by
  extension.
...
- The server and any other machine keep the old one-sided behavior until the next
...
needed windows to be FOUND, and this milestone's winbox runs are REGRESSION
...
- Reviewer ruling on the trailing-dot tightening: FLEET-WIDE, four grounds.
  ...
  SMOKED before ruling: a `find` across the real laptop store for both
  trailing-dot patterns came back empty (the server store is unverified and
...
- Only three of the four `StoreNameReason` doors are windows-reachable today
  ...
  trailing-dot evidence necessarily comes from darwin, not winbox -- a
  winbox-only four-door test would be one-third structurally unobservable, not
  incomplete by oversight.
- The darwin-visible behavior change (a trailing-dot name that worked before
  now refuses) is field-smoked safe against the current laptop store; the server
...
- Reviewer: UNCONDITIONAL final sign-off, after an initial FINDINGS verdict
  ...
  no winbox round-trip needed for them) -- `mb` killed on exactly the three
...
- D70 corrections triggered a full mutant re-run: the CORE package's build
  ...
  mutant design) and the winbox pair `m6mutT`/`m6mutP` (the transcript fragment
  and the peerEnv fragment restored respectively), both predicted single-test
  reds on winbox and GREEN on darwin -- the fragment matches unix-style paths,
  so only winbox measures it, and the asymmetry itself is the proof. All SIX
...
- Closing runs: client suite 533 RUN / 382 PASS / 3 FAIL, all three
  ...
  filed in M5, not owned by this tracker item) -- nothing this milestone touches is
  ...
  winbox" omitted the D7 exclusion by name, which read literally would have
...
- `openshared_windows.go` (new) -- `openSharedRead` via hand-rolled `CreateFile`
  ...
  measured on the TARGET VOLUME specifically (winbox NTFS: a held handle blocked
...
- `follow.go` -- `reopenUntilSuccess` (the arm-lifetime handle, the main event of
  this tracker item) now opens via `openSharedRead`.
- `formation_apply.go`, `codexstophook.go` -- their inbox-polling `os.Open` calls
  route through `openSharedRead` too.
- `store.go` -- ten `os.RemoveAll` discard sites addressed (the tracker item's own title
...
- D66 corrected the coder's first classification, which had inherited an error
  from the tracker item's own site-to-function map (it labeled two sites as prune when
...
- THE EIGHTH SITE, told in full because it took two corrections rounds and is a
  ...
  corrected that to three ways before it ever ran on winbox: removal blocks
  ...
  green). MEASURED ANSWER on winbox: row two. The rename blocks FIRST -- a held
...
- Two coder mechanism reads, tasked by the reviewer before the corrections could
  ...
  1. `TestForeignReopenIsNotStreamed`'s residual (still red at narrow scope after
     ...
     scheduling that a remove-vs-read overlap is near-certain. The original tracker item
     ...
     Filed to new tracker item `cbus-que.12`, correctly out of this milestone's scope
  ...
  2. `TestDisplacedFollowerStopsMovingTheCursor`'s flip (1/8 pre-fix to 8/8
     ...
     to new tracker item `cbus-que.10`. The tester's own initial "possible regression"
     caution is recorded as the right call to raise, with a now-measured negative
     answer.
- D68: `cmd/cbus`'s windows suite had NEVER executed on winbox before this round
  (M3 scoped its own winbox run to just the three-verb refusal trio). Rather than
  ...
  filed to new tracker item `cbus-que.11` rather than gating this milestone, since they
  are discoveries on a surface that has simply never run before, not regressions
  this tracker item introduced.
...
closed. Full record on `cbus-que.7`; findings beyond this tracker item's own scope filed
to `cbus-que.8` and `cbus-que.9`.

[Motivating problem]
Found 2026-07-26 by the tester seat by RUNNING the suite on winbox, not reading it --
...
absent from winbox PATH, verified rather than assumed. The structural liveness
...
- `fileid_windows_test.go` -- `TestFileIdentityProbeDoesNotLockOutWriters` rewritten
  ...
  not existing; it exists now and D38 is winbox-verified.
...
- Measured in passing, feeding cbus-que.8's fix design directly: from the fixmutB
  ...
  NEITHER a sequential remove NOR a 50-iteration rm+recreate loop on winbox NTFS,
...
- D60, the close-condition shape: the fix batch closed everything inside this
  tracker item's scope, but one residual cluster (three follower/cursor timeouts) proved
...
- `cbus-que.9` (new tracker item, filed from this milestone's first-execution results):
...
- Laptop: darwin suite green throughout (537 RUN / 396 PASS / 0 FAIL / 1 SKIP on
  ...
  clean, gofmt clean.
- Winbox, final closing run per D60 on the final artifact (manifest
...
- Tester, laptop half: tree-wide windows build zero diagnostics (against the tester's
  ...
  milestone.
- Tester, winbox half, run twice (pre-fold and a full rule-8 re-run post-fold), GREEN
...
- G5 mutation pass on winbox: 4 unique binary hashes (control + 3 reviewer-authored
...
- `cbus-que.7`, filed not fixed: 20+ tests build live and dead processes by spawning
  unix binaries (sleep, true, tail, /bin/sh, perl). None is on PATH on winbox, so
...
- The acceptance harness runs cross-compiled `go test -c` PE32+ binaries shipped to
  winbox, which has no Go toolchain and where no install was authorized. 487ms round
...
- Repository-subject tests (gofmt, the embed canary, selfupdate's `../../` reads) are
  permanently OUT of the winbox matrix, recorded not-attempted and never passed. Under
...
- GAPS CARRIED, stated rather than papered over. Gate 4 is OPEN: the two-process
  ...
  The `fileIdentity` share-flag fix is VERIFIED BY CONSTRUCTION ONLY with no winbox
  ...
  its premise OBSERVED on winbox (csrss returns ERROR_ACCESS_DENIED at the production
...
- The cbus-6ij epic's increments 4/5 (tracker) now trail the docs: increment 4's
...
the server's store held no peer metas at all — no pre-P3 arm survived anywhere
...
on. Field-verified nil impact on the laptop+server fleet specifically, not a
...
F1/F2 (b094b31): the first real linux runtime run (a Linux container runtime, bookworm) found
...
- Any external tooling or script that greps a NEW peer's argv for its inbox
  ...
  is not the case on the laptop or server.
...
- go build ./... and go test ./... green on darwin and in a linux/arm64
  bookworm container (a Linux container runtime) throughout the chain; F3's new case reproduced
  FAILING in the container before the fix, passing after.
- amd64 is compile-verified only (GOOS=linux GOARCH=amd64 go build ./...),
  not runtime-tested -- it rides the server deploy gate. Do not read "verified
...
[Possible Ripple Effects]
- Any still-existing pre-cutover stampless meta on laptop/server now reads dead:
...
- Not yet exercised: a live prune pass against a real pre-cutover relic
  meta (none exist on this laptop's CBUS_DIR to test against).
...
[Testing Notes]
- Repo-wide + docs-repo grep for "strips `kind`" / "presence never crosses"
...
10 docs-repo items). This was a solo documenter round -- no reviewer gate --
...
- Hosts with neither $TMUX nor $ITERM_SESSION_ID (plain SSH to the server, Terminal.app) now hard-error per pane's contract where the old template opened tabs. Deliberate, Carlos-ruled: refusing beats splitting a frontmost surface; workaround is running under tmux or copying the envelope with per-peer targets.
...
## [2026-07-18 04:12:19 UTC] [Docs] Docs-refresh effort complete: docs-repo→repo promotion + F5 + ledger (cbus-yle)
...
**docs-repo §4.12 sync-back**: ruled in separately — port-map.md's own
...
`e2c7832 8779444` (promotion + F5). Four docs-repo files refreshed direct-edit
...
reframed §5's server-propagation note for the post-cutover, post-distribution
world (`install.sh` retired, server updates via `cbus selfupdate`). That reframe
...
- `docs/architecture/cutover-decision-package.md` — rollback procedure
  (`ad45c78`).
- `docs/architecture/port-map.md` — §4.12 precision + §5 server-propagation
...
The docs-repo tier's parallel draft (index.md's "Shipped since cutover" section,
...
- docs-repo tier (cbus-yle, same effort) already finalized its mirroring
...
[Possible Ripple Effects]
- The server never-run-install.sh footgun (bash rollback) is now structurally
...
same-filesystem rename, and the server, whose tmpfs-backed `/tmp` forces the
...
the reviewer forced it deliberately — worth doing because the server's
...
`laptop` vs `laptop` machine-value display convention is deferred to an M4
ruling.
...
developer's hostname and expected STALE for it, and used a literal `"server"` as
the foreign machine — so the suite failed on any host but that one laptop, and
inverted on a host actually named server. b24cd70 fixes it: fixture machine
...
into whatever durable store the orchestrator already uses — a tracker, a markdown file,
...
- internal/client/endpoint_test.go (separate commit): TestSiteURL blanks
  CBUS_SITE_SERVER_URL so the suite passes in shells that configure the fleet.
...
- Live: branch --role refused (rc=1, teaching message); spawn --role ghost lists
  ...
  launched `ccs alpha --model sonnet --name documenter` with the doctrine
...
- Live validation on a loopback relay (laptop, `-presence-grace 2s`, production server untouched):
...
- NOT deployed to the server. The relay currently serves live `listen` sessions on
  sdk-work; a restart would blip those four tails. Deploy in a quiet window via
  relay/deploy.sh (build-on-server), backward-compatible, no client swap needed.
...
the `@server` relay. Root cause: the relay's `/peers` view is the union of the
...
- `go build ./... && go vet ./...` clean. `go test ./...` green (the one
  `TestSiteURL` failure is a pre-existing env leak — `CBUS_SITE_SERVER_URL` is set in
  Carlos's shell; `env -u CBUS_SITE_SERVER_URL go test ./...` is fully green — and it
...
- Live HTTP smoke against a loopback relay (127.0.0.1:18090, temp spool+token):
  ...
  was rmdir'd (only `c` left). Client `cbus prune foo@server/bar` → `prune is
  channel-scoped: use <channel>@server or @server`, exit 1, BEFORE any keychain/network
  touch.
- NOT redeployed to the server. The relay binary there is pinned (orchestrator quiesce
...
- docs/prior-art-and-cc-internals.md — §1 sibling project names / GitHub handles /
  ...
  dashboard-bug bullet + both `scratchpad/` file paths (cbus-foc / cbus-oq9
...
- `go test -race -count=1 ./...` green; gofmt clean. Smoke via scratchpad binary:
  ...
  at 1a5821d until the quiesce window (rebuilding now would break @server for live
  sessions lacking CBUS_SITE_SERVER_URL), so a real fork would exercise the old
  binary anyway. Validate child join-reclaim live after the window's redeploy.

## [2026-07-14 04:50:45 UTC] [Port/Go] Generalize relay-host resolution — drop the built-in `server`
...
`UnknownHostError` (previously `server` fell through to a hardcoded base).
...
the Go client, so a rollback also requires `CBUS_SITE_SERVER_URL` to be set (already
provisioned on the laptop; the server is loopback-served and needs it only as insurance).

[Files Changed]
- internal/client/endpoint.go — `SiteURL` loses the `switch host { case "server" }`
  built-in; returns the env override or `UnknownHostError`. Doc comment updated.
- bin/cbus — `site_public_url` loses the `server)` case; falls straight to `die`.
  Comment updated ("no built-in hosts").
- internal/client/endpoint_test.go — `TestSiteURL` now asserts server-without-override
  is `UnknownHostError` (not a base) plus the override + unknown-host cases;
  `TestResolveFrontDoor` sets `CBUS_SITE_SERVER_URL` so the public-mode branch resolves.
- docs/architecture/{command-reference,overview,protocol}.md — the "built-in table:
  only server" claims annotated as the since-retired port-verified behavior.
...
[Possible Ripple Effects]
- Any client that addressed `@server` via the built-in now REQUIRES
  `CBUS_SITE_SERVER_URL` in its environment. Running sessions carry their start-time
...
- Committed after Carlos released the hold (push-review was waiting on THIS
  commit; orchestrator notified with the hash on go-port@server). Server propagation
...
- `go test -race -count=1 ./...` green; gofmt clean. Deployed to laptop + server
...
- 6 new unit tests (exact scope not itemized in the commit message beyond
  count).
- **Deployed**: both the laptop and server `cbus` binaries rebuilt at this
...
- **`1ee6f46`** — `commands/bus-join.md` softens the "nothing is
  ...
  `~/.claude/commands/` on the laptop and propagated to the server via `scp`
...
- `bus-join.md`'s propagation to both machines was checksum-verified
  (`md5`/`md5sum` identical on laptop and server) rather than assumed.
...
- `docs/architecture/compat-deletion-plan.md` (new) — enumerates the
  ...
  homogenization.
- **Evidence**: Class A/B differential sweep, 27/27, run on BOTH laptop
  (darwin/arm64) and the server (linux/amd64, via a temp binary — nothing
...
- **Window leg** (a real `ccs` fork, not a probe): the terminal window
  ...
  recorded; env correctly replicated (alpha profile, cwd,
...
- New regression passes on both darwin (laptop) and linux (server).
...
- Validated on BOTH platforms: self-parse `procArgs(self) == os.Args`
  (condition (i)) on laptop darwin and server linux; the real-process liveness
...
- **server live local-mode differential**: coder cross-compiled a temp
  ...
  permission boundary the reviewer doesn't have (no server shell access) — and
...
- **First live public-mode differential test**: ran `list p1diff@server` and
  `send --from EQ ...` through both bash `cbus` and `cbus-go` against the
  real server relay — byte-identical output on both. Leaves throwaway
...
- `internal/client/endpoint.go`, `endpoint_test.go` (new) — `SiteURL` (built-in
  `server` host table + `CBUS_SITE_<HOST>_URL` env-mangling, keeping the
...
- `relay/internal/conformance/conformance_test.go`, `doc.go` (new) — hermetic
  ...
  `~/.claude-bus`, no server contact) and drives `POST /send`, `GET /peers`,
...
- Carry-forward for P1: the conformance rig's build cache can go stale
  ...
  deploy.sh server-cleanup check.
...
- `relay/deploy.sh` — reviewer note n1: added a guarded `rm -rf` of the pre-restructure
  ...
  leaves stale on the server (and the stale cmd/ still compiles under the new module).
...
- `relay/deploy.sh` — retargeted for the root-module layout: rsync now ships the repo
  root's `go.mod` + `internal/` + `relay/` (keeps the module self-contained on the server);
...
- deploy.sh simulation (orchestrator-required, so the reviewer can check the layout
  ...
  `src/` mirroring the server layout, then ran the exact deploy build commands against it.
  Native (`go build ./relay/cmd/cbus-relay` + wstail) → OK; server-target cross-compile
  (`GOOS=linux GOARCH=amd64`) → OK (ELF x86-64, statically linked).
- Go toolchain: none existed on the laptop (the relay was always built on the server via
...
- Pure documentation; no runtime behavior changed. No server propagation needed
...
- ~27 open questions unanswerable from code (CF Access config, live Monitor cap
  drift, server spool state, SessionEnd-on-/clear semantics) are listed in
  port-map.md §8 / index.md rather than guessed at.

[Attempt #2 — follow-up to the presence commit]

A second adversarial diff review by the Fable-5 peer (`dev@server/reviewer`) against
...
[reviewer #7]. Server propagation [reviewer #8] already done.
...
(`dev@server/reviewer`); all 10 findings folded in (see the plan file).
...
- The server runs its own cbus copy for loopback bridge peers → must re-run
  `install.sh` there or server-local prune/presence diverges [reviewer #10].
...
- `go vet ./...` clean + `go test ./cmd/cbus-relay/` all pass on the server (isolated
  /tmp build) BEFORE deploying — deploy.sh aborts on build failure (set -e) so a
  bad build never reaches systemd.
- Live server->relay->laptop after the real restart: 2429-char message arrived framed
...
- Prune: dead-owner marker swept, live-owner kept, legacy file swept, empty
  dirs cleaned. PASS.
- Live: this session's real legacy marker (bridge@server) migrated by re-arming;
...
- Static check: no curl call site carries Authorization/CF-Access material in
  argv. PASS.
- Live public loop with the new path: `cbus list @server` through the CF front
...
- `bin/cbus`
  ...
  - `cmd_auth` (`set`/`status`): credentials in macOS Keychain via security(1)
    ...
    under ~/.config/cbus/ on Linux (server has no Keychain). `-` reads stdin;
    status prints masked last-4 only. No tokens in code — per the standing
    requirement; ssh-reading the server token file remains dev-only.
...
- Live (ssh -L 8090 = the documented dev/local mode): send queued pre-tail →
  ...
  event; live send delivered with marker-based routable from=probe@server/laptop;
...
- `README.md` — "Networked relay (server)" section.
...
On-server suite (loopback :8091, throwaway spool): healthz; 401 unauth; queue→new/;
