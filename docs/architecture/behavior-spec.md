...
> Mac carried a `listenerStart` (9 of 9), and the server's store held no peer metas
...
| `list`/`peers` `[--active|-a] [ch \| [ch]@host]` | :580-612 | Local row: `listen|off  <ch>/<al>  pid=<p>  <host>  <cwd>` (`%-28s` address col). Legacy v1 rows advertised for prune. Never prunes. Remote (`@` in **$1** only): renders `/peers` → `listen|off  <ch>@<host>/<al>  queued=N lastSeen=…`. **No active-only remote view exists by any arg order**: `list ch@server --active` silently discards trailing args (:296 reads $1 only); `list --active ch@server` treats `ch@server` as an unmatchable local filter → misleading `no active listeners`; `active ch@server` is structurally dead (:842 prepends `--active`) |
...
| `auth status [host=server]` | :762-770 | `set (…last4)` / `absent` per field; never full secrets; **always exit 0**. **Quirk**: host arg is the only unvalidated `auth_get` entry — Linux `../` traversal reads arbitrary `token/cf-id/cf-secret` files (masked); values <4 chars display `set (…)` empty under bash |
