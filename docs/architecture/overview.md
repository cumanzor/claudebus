...
- **Addressing**: remote targets are `<channel>@<host>/<alias>`. The `@host` maps to *which relay
  site to talk to* (host table: `server → https://bus.example.com`, extendable via
...
- **Remote receive** is not a process: `cbus tail <ch>@<host>/<alias>` *prints* a Monitor `ws:`
  ...
  streams live.
- **Aliases are explicit** remotely (short hostname/role, e.g. `laptop`, `server`). There is no remote
...
sequenceDiagram
    ...
    C-->>S: sent to dev@server/server via public relay (from dev@server/laptop)
...
- **Copy-install drift**: `install.sh` copies by default, so the server's client copy must be
  re-installed after any `bin/cbus` or `commands/` change (the relay has its own deploy path,
  `relay/deploy.sh`, which rsyncs source and builds on the server with its Go toolchain). Nothing
...
feature was adversarially reviewed by a peer session reached at `dev@server/reviewer` *through the
cross-machine relay*.
...
- **No spool GC anywhere**: `cur/` accumulates every delivered message forever, `tmp/` orphans are
  ...
  monotonically, and dead test channels are immortal. Verified live on the server: no cron, no timer.
...
- Copy-install drift between machines is a real failure mode; protocol changes require re-running
  `install.sh` on the server by hand.
