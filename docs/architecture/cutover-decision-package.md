...
> was performed on the laptop and the server — `~/.local/bin/cbus` on both is now the Go
...
> Current wire/behavior contract: [protocol.md](protocol.md).
...
- **Class A/B differential sweep — 27/27 on BOTH platforms** (laptop darwin/arm64, server
...
- **Window leg** — the iTerm2 window **opened and stayed open** with a live
  ...
  (`ccs alpha --resume <sid> --fork-session <bootstrap prompt>`); env replicated
  (`alpha` profile, cwd, `CLAUDE_CONFIG_DIR`); the launcher tmpfile **self-deleted**.
- **Tmux leg** — a `cc-branch` tmux window was created and ran the launcher, spawning
  ...
  the fork's `ps -wwE`: `CLAUDE_CONFIG_DIR=~/.ccs/instances/<profile>`, replicated PATH).
...
**Rollout order** (per the effort's standing plan): **laptop first** (richest usage),
then **server** (re-run its installer — copy-install does not auto-update), then
**winbox/WSL** starts on the port directly. Each machine is independent — the shared
...
| Gate | Result | Source |
|------|--------|--------|
| Class A/B sweep, laptop | 27/27 | `scripts/p26_sweep.sh` |
| Class A/B sweep, server (linux/amd64) | 27/27 | `scripts/p26_sweep.sh` via rsync-to-tmp + build + run + delete |
