...
> `cbus` on the laptop and the server is the Go binary (27/27 differential parity; decision
...
>    the server runs its own hand-updated copy, and a third node (winbox/WSL, tracked as `cbus-dc5`)
...
- **Copy-install drift across machines is a standing failure mode.** Default install mode is
  `cp`; after any `bin/cbus` or `commands/` change the server copy must be re-installed by hand or
  "server-local prune/presence diverges" (detailed_changelog; user memory note). No version stamp
...
| **M12 install & deploy** | install.sh, server propagation, version stamping | C | install.sh; relay/deploy.sh |
...
flowchart TB
    ...
    subgraph server["server (unchanged)"]
...
| 24 | systemd on the server / **nothing** on the laptop (relay/cbus-relay.service; absence) | The relay is supervised; the client is deliberately zero-daemon — the only long-lived client processes are Monitor-managed followers | Unchanged. Any client-side daemon (e.g. an auto-reconnect ws bridge) is a deliberate architecture change: launchd + a resident process holding the relay token | Sneaking in a daemon changes the trust story and the "nothing to babysit on the laptop" property |
...
ends with a manual `install.sh` re-run on the server (copy-install; per the standing memory note,
skipping it means server-local prune/presence diverges). The port's installer should: embed a
...
2026-07-13 (laptop + server).** The core of the port.
...
cc-branch.sh). Installer gains the version stamp and hook-wiring check. Rollout order: laptop
first (richest usage, fastest feedback), then the server (propagate per the memory note), then
the winbox/WSL node starts on the port directly. Gate: the full Class A/B registry of §5,
...
| **Swift** | No | macOS-native Keychain and process APIs, but the fleet is majority-Linux (server, WSL) and the cross-compile/deploy story there is the project's weakest axis |
...
7. The WSL/winbox node (`cbus-dc5`): procfs is fine, but does its filesystem of choice share
