...
>    (retired `de07cbe`; the server now updates via `cbus selfupdate`), and a third node (winbox/WSL,
...
historical. A server client update is now `cbus selfupdate` (which also refreshes `commands/` and
