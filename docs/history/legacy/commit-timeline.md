...
| go-port (epic cbus-8k9) | P0 shared core → P1 remote verbs (side-by-side `cbus-go`) → P2 local transport/follower/native branch → P2.6 readiness (27/27 both platforms) → **cutover executed 2026-07-13 (laptop + server)** |
...
| 07-14 | `eaa12e6` | Relay-host resolution generalized: env-only `CBUS_SITE_<HOST>_URL`, built-in `server` table dropped |
